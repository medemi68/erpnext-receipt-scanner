...
class Invoice2ErpnextLog(Document):
    ...
    def _create_invoice_item(self, item_code, quantity, unit_price, amount, description, is_credit):
        ...
        For negative line items (discounts/credits), we keep qty positive and rate negative
        so that qty * rate = amount (negative). This ensures _adjust_item_prices works correctly.
        """
        ROUNDING_TOLERANCE = 0.05
        credit_desc = f"CREDIT: {description}" if is_credit and not description.startswith("CREDIT:") else description
        abs_qty = abs(quantity) if quantity else 1
        ...
        if unit_price and quantity and amount:
            ...
            if abs(calculated_amount - amount) > ROUNDING_TOLERANCE:
                # Mismatch - derive rate from amount to keep qty * rate = amount
                effective_rate = self._round_amount(amount / abs_qty) if abs_qty else amount
                return {
                    "item_code": item_code,
                    "qty": abs_qty,
                    "rate": effective_rate,
                ...
                # unit_price * quantity matches amount - keep rate consistent
                # For credits, unit_price is already negative from extraction
                return {
                    "item_code": item_code,
                    "qty": abs_qty,
                    "rate": unit_price,
            ...
            calculated_amount = self._round_amount(unit_price * abs_qty)
            return {
                "item_code": item_code,
                "qty": abs_qty,
                "rate": unit_price,
